...
function reminder_v11(email,text) {
    ...
    if((tYyyy + tMm + tDd == rYyyy + rMm + rDd) || (email_test == 1)){
      ...
      for (var k = 0; (k <= actual_TaskColumns - 1); k++) {
        ...
        console.log("No need to send reminder email to speaker " + duty_contacts[i][k] +" !!!");
          ...
          msg_alert = msg_alert + "\nALERT!!! \""+duty_contacts[i][k] +"\" is NOT in the av_contact list, hence NO reminder email/text was send to him/her, Please 1)Check spelling(case sensitive) or 2)Update av_contact list accordingly !!!\n";
          console.log("BAD NEWS!!! Can't find " + duty_contacts[i][k] +" in the av_contact list !!!");
        }
        else if (email_collection.get(duty_contacts[i][k]) == "") {
          msg_alert = msg_alert + "\nALERT!!! No email address was found for \"" + duty_contacts[i][k] +"\" in the av_contact list, hence NO reminder email was send to him/her, Please update av_contact list accordingly!!!\n";
