...
function reminder_v13(test,email,text) {
  
  //when test is 0, running it in real mode, either sending text when 1 or email when 1
...
  
  ...
  for (var i = 0; ((i <= actual_numRows - 1) && (find_it == 0)); i++) {
    ...
    if((tYyyy + tMm + tDd == rYyyy + rMm + rDd) && (run_email == 1)){
    ...
    else if ((tYyyy + tMm + tDd == xYyyy + xMm + xDd) && (run_text == 1)) {
